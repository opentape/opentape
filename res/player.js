    ...
    for(i = 0; i < openPlaylist.length; i++) {
        ...
        if (trackEntry) {
            ...
            trackEntry.addEvent('click',function(e) {
                ...
                if (targ.id.indexOf("song")!=-1) {
                    togglePlayback(targ.id);
                }
                else if (targ.parentNode.id.indexOf("song")!=-1) {
                    togglePlayback(targ.parentNode.id);
                }
                else if (targ.parentNode.parentNode.id.indexOf("song")!=-1) {
                    togglePlayback(targ.parentNode.parentNode.id);
                }
                else if (targ.parentNode.parentNode.parentNode.id.indexOf("song")!=-1) {
                    togglePlayback(targ.parentNode.parentNode.parentNode.id);
                }
            });
        }
    }
...
function playTrack() {
    ...
    try {
        playerObj[0] = soundManager.createSound({
        ...
        debug('Cant create sound: ' + err.description );
    }
    setupTrackDisplay(currentTrack);
}
...
function loadNextTrack() {
    ...
    if (!openPlaylist[(currentTrack+1)]) {
        debug("This is the last track");
        return false;
    }
    if (playerObj[1]) {
        playerObj[1].destruct();
    }
    try {
        playerObj[1] = soundManager.createSound({
...
function sm_onload() {
    if (currentPlayerObj.readyState == 2) {
        nextTrack();
    }
}
function sm_onplay() {
...
function sm_whileplaying() {
    if (this.sID != "playerObj" + currentTrack) {
        return false;
    } // ignore timing events for players that are fading in
    player_position = parseInt(this.position/1000);
    player_duration = parseInt(this.duration/1000);
    if (player_position==currentPos) {
        return false;
    }
    else {
        var string = '';
        ...
        if (is_fade_enabled() && (player_duration - player_position) == 10) {
            loadNextTrack();
        }
        else if (is_fade_enabled() && (player_duration - player_position) == 5) {
            beginFadeTransition();
        }
    }
}
