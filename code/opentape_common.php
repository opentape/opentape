...
$cwd = getcwd();
if (preg_match('/code\/?$/', $cwd)
    || preg_match('|' . constant("SETTINGS_PATH") . '?$|', $cwd)
    || preg_match('/res\/?$/', $cwd)) {
    chdir('..');
}

// This function transforms the php.ini notation numbers (like '2M')
...
function is_password_set() {
    $password_struct = get_password_struct();
    if (is_array($password_struct)
        && !empty($password_struct)
        && $password_struct !== false) {
        return true;
    }
    else {
        return false;
...
function get_songlist_struct() {
    ...
    && is_readable(constant("SETTINGS_PATH") . $filename_base . '.php')) {
        // this is the more secure way of storing this data,
        // as the web users can't fetch it
