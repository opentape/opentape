...
foreach ($songlist_struct as $pos => $row) { 
    ...
    if (isset($row['opentape_artist'])) {
        $creator->appendChild($doc->createTextNode($row['opentape_artist']));
    }
    else {
        $creator->appendChild($doc->createTextNode($row['artist']));
    }
    $trackitem->appendChild($creator);
    $title = $doc->createElement('title');
    if (isset($row['opentape_title'])) {
        $title->appendChild($doc->createTextNode($row['opentape_title']));
    }
    else {
